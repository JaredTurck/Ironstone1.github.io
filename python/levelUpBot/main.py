...
for username in inbox:
    ...
    for line in lines:
        if "!check" in line:    # !check - checks to see if the bot is online and ready to serve you.
            ...
            driver.execute_script("document.querySelectorAll('form[id=\"chatform\"] button[type=\"submit\"]')[0].click();")

        if "!help" in line:
            driver.execute_script("document.getElementById(\"chatmessage\").value = \""+commands["!help"]+"\";")
            driver.execute_script("document.querySelectorAll('form[id=\"chatform\"] button[type=\"submit\"]')[0].click();")
        ...
        if "!buy" in line:
            ...
            #   select Steam/Game
            driver.execute_script("TradePageSelectInventory( UserYou, "+str(AppId)+", 0 );")
            Wait4Elm("document.getElementsByClassName(\"itemHolder\")[0]", False)
            
            #   search for the item
            # document.querySelectorAll('div[id^="item753_6_7619058500"][class^="item app753"]')[0]
