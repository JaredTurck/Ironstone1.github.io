...
class http():
    def GET(conn, request, addr, custom_404=True):
        # send the file
        url = "https://jaredbot.uk"
        file_to_send = get_proxy_data(url)
        
        data_type = "text/html"
        head = f"HTTP/1.1 OK "+\
        "date: " + datetime.datetime.now().strftime("%a, %d %B %Y %H:%M:%S GMT")+\
        f"Content-Length: {len(file_to_send)} Connection: close Content-Type: {data_type} "+\
        " cache-control: max-age=3600 CF-Cache-Status: DYNAMIC Content-Encoding: gzip "
        data = head.encode('UTF-8') + file_to_send
        
        conn.sendall(file_to_send)
        print(f"[+] Sent file '{url}' to {addr[0]}!")
