    ...
    def write2video(self):
        for i in range(self.avg_fps):
            if self.current_frame in self.frames.keys():
                # compress frame
                buffer = BytesIO()
                self.frames[self.current_frame].save(buffer, "JPEG", quality=self.frame_quality)

                # write frame
                img_array = cv2.imdecode(numpy.frombuffer(buffer.getvalue(), numpy.uint8), -1)
                
                self.video.write(img_array)
        ...
        while self.exit == False:
            ...
            if keys != []:
                if keys[0] in self.frames.keys():
                    buffer = BytesIO()
                    self.frames[keys[0]].save(buffer, "JPEG", quality=self.frame_quality)
                    print(True)
