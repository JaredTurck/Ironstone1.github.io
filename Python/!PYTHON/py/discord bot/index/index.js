		...
		if (sniping == true) {
			try {
				...
				if (msg.member != null) {
					...
					console.log("Failed to write deleted message to log file!")
				}
			} catch (err) {
...
function play_song(msg, channel, forceplay=false, forceplay_url="") {
		...
		channel.join().then(connection => {
			...
			function song_manager() {
				...
				if (forceplay == true) {
					...
					dj = connection.play(stream[msg.guild.id], steamOptions);
				} else {
				...
				dj.on("finish", song_finished => {
					...
					if (song_queus[msg.guild.id][0] == undefined) {
						...
						console.log("queue ended for " + channel);
					} else {
						song_manager();
					}
				})
...
function song_info(msg, response, yt_url) {
	...
	//	embed_error(msg, "There are no songs currently in the queue!");
	//	return false;
	//}
	...
	song_description = response.items[0].snippet.description.slice(0, 2048);
...
bot.on("message", msg => {
	if (authrosied_server_IDs.indexOf(msg.guild.id) > -1) {
		...
		if (msg.content.slice(0, 6) == prefix+"play " || msg.content.slice(0, 11) == prefix+"forceplay ") {
			command = msg.content.slice(6, msg.content.length);
			
			// check for forceplay
			if (msg.content.slice(0, 11) == prefix+"forceplay ") {
				forceplay = true;
			} else {
				forceplay = false;
			}
			...
			// find song by name
				...
				youtube_api_url = "https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=1&q=";
				url = youtube_api_url + command + "&key=" + YouTube_Data_API;
				console.log([url]);
				...
				request(url, {
					...
					console.log("Youtube Data API response code " + res.statusCode);
					if (res.statusCode == 200) {
						response = JSON.parse(body);
						...
						if (song_player[msg.guild.id] != true || forceplay == true) {
							song_player[msg.guild.id] = true;
							play_song(msg, channel, forceplay=forceplay, forceplay_url=song_url);
							console.log(["force play", forceplay]);
						}
						
						// embed
						song_info(msg, response, song_url);
					} else if (res.statusCode == 403) {
						msg.channel.send(msg, "YouTube data API responded with error code 403! The HTTP 403 Forbidden client error status "+
						"response code indicates that the server understood the request but refuses to authorize it. to many requests might "+
						"of been sent to the server, try again latter!");
					}
				});
			}
		...
		// - ping (checks the bots response time to discord)
			...
			msg.channel.send("Pinging...").then(res => {
